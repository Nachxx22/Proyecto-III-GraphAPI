...
public class Lista {
	...
	/***
	 * @see Nodo cabeza, es el primer nodo de la lista enlazada
	 */
	public Lista() {
		cabeza = null;
		setSize(0);
			...
			for (int i = 0; i < lista.size; i++) {
				if (lista.ver(i) != null && lista.ver(i) instanceof Edge) {
					if (((Edge) lista.ver(i)).destination == null) {
						System.out.println("\n Edge [origin=" + ((Edge) lista.ver(i)).origin.getName()
								+ ", destination= null" + ", distance=" + ((Edge) lista.ver(i)).distance + "]");
					} else {
						System.out.println("\n Edge [origin=" + ((Edge) lista.ver(i)).origin.getName()
								+ ", destination=" + ((Edge) lista.ver(i)).destination.getName() + ", distance="
	...
	/***
	 * @param index indice en la lista a eliminar Elimina nodos por medio de la
	...
	/***
	 * @param indice indice donde se encuentra el elemento que se busca observar el
	 *               elemento de la lista usando el indice dode se encuentra
